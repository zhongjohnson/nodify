...
namespace Nodify
{
    ...
    // PORT: copied from upstream Nodify\Editor\NodifyCanvas.cs. The ONLY change vs. upstream is
    // `children[i].RenderSize` -> `children[i].Bounds.Size` (Avalonia's Control has no WPF `RenderSize`
    // property; `Bounds.Size` is the arranged size). Kept as a copy (not a linked source) so this single
    // adaptation is isolated for the next upstream merge.
    public class NodifyCanvas : Panel
    {
        ...
        protected override Size ArrangeOverride(Size arrangeSize)
        {
            ...
            for (int i = 0; i < children.Count; i++)
            {
                ...
                Size size = children[i].Bounds.Size; // PORT: upstream reads children[i].RenderSize
